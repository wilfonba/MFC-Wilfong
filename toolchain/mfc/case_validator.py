...
class CaseValidator:
    ...
    def check_interface_compression(self):
        ...
        if recon_type == 1:  # WENO
            weno_order = self.get("weno_order")
            self.prohibit(weno_order == 1 and int_comp in [1, 2], "int_comp must be 0 (off) when weno_order = 1")
        elif recon_type == 2:  # MUSCL
            muscl_order = self.get("muscl_order")
            self.prohibit(muscl_order == 1 and int_comp in [1, 2], "int_comp must be 0 (off) when muscl_order = 1")
