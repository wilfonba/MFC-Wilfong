...
| `int_comp`                 | Integer | Interface Compression [0] Off [1] THINC [2] MTHINC [3] CDI sharpening [4] ACDI sharpening (default 0) |
...
- `int_comp` activates interface compression using [1] THINC or [2] MTHINC (default off) used in variable reconstruction, with control parameters (`ic_eps`, and `ic_beta`), or [3]/[4] conservative diffuse-interface (CDI/ACDI) sharpening, which instead adds conservative regularization fluxes to the governing equations that drive interfaces to a fixed equilibrium thickness (five-equation model only). ACDI computes the sharpening normal from a signed-distance-like variable, which reduces grid-aligned shape distortion of small droplets and is generally preferred.
