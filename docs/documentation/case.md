...
| `added_mass_force`    | Logical | Enable added mass force (default false)                        |
...
- `vel_model` activates translational motion of the bubbles (\cite Wilfong26): [1] tracer bubbles, which are advected with the local carrier velocity \f$\underline{u}\f$ so that \f$\dot{\underline{x}}_b = \underline{u}(\underline{x}_b)\f$; [2] Newton's second law, which integrates \f$m_b \ddot{\underline{x}}_b = \underline{F}_D + \underline{F}_p + \underline{F}_g + \underline{F}_A\f$, where \f$m_b\f$ is the bubble mass and the right-hand side collects the drag, pressure, gravity, and added-mass forces below. The carrier velocity at the bubble is interpolated with a Lagrange polynomial of order set by `fd_order`, which must be specified when `vel_model > 0`.

    - `drag_model` selects the drag force \f$\underline{F}_D\f$ acting on the slip velocity \f$\underline{u}_{\rm rel} = \underline{u}_b - \underline{u}\f$, with bubble radius \f$a\f$ and Reynolds number \f$Re\f$: [0] no drag (default); [1] free-slip drag (clean-interface creeping-flow limit, \cite Hadamard1911, \cite Rybczynski1911), \f$\underline{F}_D = -4\pi a\,\underline{u}_{\rm rel}/Re\f$; [2] no-slip Stokes drag (rigid sphere, \cite Stokes1851), \f$\underline{F}_D = -6\pi a\,\underline{u}_{\rm rel}/Re\f$; [3] Levich drag (clean bubble at high \f$Re\f$, \cite Levich1962), \f$\underline{F}_D = -12\pi a\,\underline{u}_{\rm rel}/Re\f$; [4] finite-Reynolds-number clean-bubble correlation of \cite Mei1994, \f$\underline{F}_D = -4\pi a\,\underline{u}_{\rm rel}/Re \left[1 + \left(8/Re_b + \frac{1}{2}\left(1 + 3.315\,Re_b^{-1/2}\right)\right)^{-1}\right]\f$ with bubble Reynolds number \f$Re_b = 2 a \rho |\underline{u}_{\rm rel}| Re\f$, which recovers [1] as \f$Re_b \to 0\f$ and [3] as \f$Re_b \to \infty\f$. See \cite Magnaudet2000 for a review of these bubble-drag regimes.
    ...
    - `added_mass_force` (default false) enables the added-mass force in the momentum form for a bubble of changing volume, \f$\underline{F}_A = -\frac{1}{2}\rho\,\frac{d}{dt}\left[V_b\,\underline{u}_{\rm rel}\right] + \frac{1}{2}\rho V_b\,D\underline{u}/Dt = \frac{1}{2}\rho V_b\left(D\underline{u}/Dt - \ddot{\underline{x}}_b\right) - \frac{1}{2}\rho\,\dot{V}_b\,\underline{u}_{\rm rel}\f$ (\cite Auton1988, \cite Magnaudet2000), with \f$\dot{V}_b = 4\pi a^2 \dot{a}\f$ and the carrier acceleration taken from its inviscid momentum balance, \f$D\underline{u}/Dt = -\nabla p/\rho + \underline{g}\f$. The \f$\ddot{\underline{x}}_b\f$ part is absorbed into the bubble inertia, \f$m_b \to m_b + \frac{1}{2}\rho V_b\f$; the \f$\dot{V}_b\f$ term is what keeps the cycle-averaged force balance of a radially oscillating bubble free of a spurious mean drift. Requires `vel_model = 2` and `pressure_force = T`.
