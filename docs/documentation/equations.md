...
The sharpening (anti-diffusion) term is applied only at faces where the pairwise fraction is locally monotone along the flux direction (a THINC-style monotonicity gate); elsewhere only the diffusion term acts, so under-resolved non-monotone features decay instead of being steepened into grid-scale oscillations.
