...
| Added mass \f$\mathbf{F}_A\f$ | `added_mass_force` | \f$-\frac{1}{2}\rho\,\frac{d}{dt}\left[V_b(\mathbf{u}_b - \mathbf{u}_l)\right] + \frac{1}{2}\rho V_b\,D\mathbf{u}_l/Dt = \frac{1}{2}\rho V_b\left(D\mathbf{u}_l/Dt - d\mathbf{u}_b/dt\right) - \frac{1}{2}\rho\dot{V}_b(\mathbf{u}_b - \mathbf{u}_l)\f$, with \f$D\mathbf{u}_l/Dt = -\nabla p/\rho + \mathbf{g}\f$ from the inviscid carrier momentum balance (\cite Auton1988, \cite Magnaudet2000); the \f$d\mathbf{u}_b/dt\f$ part is absorbed into the bubble inertia and the \f$\dot{V}_b\f$ term accounts for the changing bubble volume |
