...
#:endif
    ...
    subroutine s_save_performance_metrics(t_step, time_avg, time_final, io_time_avg, io_time_final, proc_time, io_proc_time, file_exists, start, finish, nt)
        ...
        if (proc_rank == 0) then
            ...
            grind_time = time_final*1.0e9_wp/NINT(1._wp*sys_size*maxval((/1,m_glb/))*maxval((/1,n_glb/))*maxval((/1,p_glb/)))
...
!&>
    ...
    subroutine s_initialize_gpu_vars
        ...
        !$acc update device(nb,viscous,R0ref, Ca, Web, Re_inv, weight, R0, V0, bubbles_euler, polytropic, polydisperse, qbmm, R0_type, ptil, bubble_model, thermal, poly_sigma, adv_n, adap_dt, n_idx, pi_fac, low_Mach)
